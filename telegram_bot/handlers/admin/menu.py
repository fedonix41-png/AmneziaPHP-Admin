...
async def cb_admin_servers_menu(callback: CallbackQuery) -> None:
    ...
    if error:
        await _safe_edit(callback, error, reply_markup=admin_main_kb(), parse_mode=None)
        return

    if not servers:
        await _safe_edit(callback, "ℹ Нет серверов.", reply_markup=admin_main_kb())
        return
...
async def cb_admin_clients_manage(callback: CallbackQuery) -> None:
    ...
    if error:
        await _safe_edit(callback, error, reply_markup=admin_clients_menu_kb(), parse_mode=None)
        return
...
async def cb_admin_mgmt_server(callback: CallbackQuery) -> None:
    ...
    await callback.answer()  # acknowledge immediately, before API call
    _store_server(_uid(callback), server_id)
    clients: list = []
    error = None
    try:
