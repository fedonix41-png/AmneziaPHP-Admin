...
*   ✅ `Получение файлов` настроек (QR-код и `.conf`). Поддержка формата `vpn://` (через специальный QR-код или конфиг) для новых протоколов.
...
*   ✅ `Управление клиентами`: поиск, блокировка (`revoke`), активация (`restore`), удаление, изменение срока и лимитов трафика, просмотр QR-кода (в том числе `vpn://`) и конфига.
...
*   ✅ `Управление серверами`: добавление (`POST /api/servers`), удаление (`DELETE /api/servers/{id}`), запуск селф-тестов и диагностики рукопожатий на серверах, а также создание клиентов напрямую с экрана управления сервером.
